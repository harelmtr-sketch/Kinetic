    ...
    <Canvas style={{ width: canvasSize.width, height: canvasSize.height }}>
      ...
      <Group transform={sceneTransform}>
        ...
        {edgeSegments.map((edge) => {
          ...
          const boostedO = Math.min(0.92, o + (edge.status === 'locked' ? 0.02 : 0.08));
          ...
          return (
            <Group key={edge.id}>
              ...
              <Path path={edge.path} style="stroke" strokeWidth={w} color={color} strokeCap="round">
                {edge.status !== 'mastered' && !bld && <DashPathEffect intervals={dashIntervals} />}
              </Path>
            </Group>
          );
        })}
        ...
        {visibleNodes.map((n) => {
          ...
          const auraOpacity = status === 'locked' ? (isFarNode ? 0.18 : 0.12) : (isReady ? 0.28 : 0.22);
          const auraColor = toRGBA(visual.stroke, auraOpacity);
          const auraR = isFarNode ? NODE_R * 0.74 : (isLit ? NODE_R * 1.02 : NODE_R * 0.9);
          return (
            <Group key={n.id}>
              {!isFarNode && <Circle cx={rx} cy={ry} r={NODE_R * 1.22} color={visual.ambient || toRGBA(visual.stroke, 0.03)} />}
              ...
              {!isFarNode && isLit && USE_GLOW && (
                <Circle cx={rx} cy={ry} r={NODE_R * 1.34} color={toRGBA(visual.glowOuter || visual.stroke, 0.12)} />
              )}
              {LOD.showNodeGlowBlur && !isInteracting && USE_GLOW && isLit && !isFarNode && (
                <Group>
                  <Circle cx={rx} cy={ry} r={NODE_R * 1.04} color={visual.glowOuter}><Blur blur={GLOW_QUALITY === 'high' ? 18 : 12} /></Circle>
                  <Circle cx={rx} cy={ry} r={NODE_R * 0.76} color={visual.glowInner}><Blur blur={4.6} /></Circle>
                </Group>
              )}
              {isFarNode ? (
                <Group>
                ...
                <Group>
                  ...
                  {!LOD.simplifyNodeStack && <Circle cx={rx} cy={ry} r={renderR - 4.6} color={visual.innerFill} opacity={0.95} />}
                  <Circle cx={rx} cy={ry} r={NODE_R * 0.34} color={visual.core || toRGBA(visual.stroke, 0.14)} />
                  <Circle cx={rx} cy={ry} r={renderR - 6.7} style="stroke" strokeWidth={nodeStrokeWidth} color={visual.stroke} opacity={visual.opacity} />
                  {LOD.showInnerRing && <Circle cx={rx} cy={ry} r={NODE_R - 13.4} style="stroke" strokeWidth={0.95} color={visual.innerRing || visual.ring} opacity={0.58} />}
              ...
              {LOD.showLabels && lines.map((ln, li) => {
                ...
                const glow1 = isLit ? toRGBA(visual.stroke, 0.32) : 'rgba(96,84,70,0.16)';
                return (
                  <Group key={`${n.id}_${li}`}>
          ...
          <Group transform={draggedTransform}>
            ...
            {LOD.isFar ? (
              ...
              <Group>
                ...
                {!LOD.simplifyNodeStack && <Circle cx={0} cy={0} r={NODE_R - 4.6} color={draggedNodeMeta.visual.innerFill} opacity={0.95} />}
                <Circle cx={0} cy={0} r={NODE_R * 0.34} color={draggedNodeMeta.visual.core || toRGBA(draggedNodeMeta.visual.stroke, 0.14)} />
                ...
                {LOD.showInnerRing && <Circle cx={0} cy={0} r={NODE_R - 13.4} style="stroke" strokeWidth={0.95} color={draggedNodeMeta.visual.innerRing || draggedNodeMeta.visual.ring} opacity={0.58} />}
            ...
            {LOD.showLabels && draggedNodeMeta.lines.map((ln, li) => {
              const glow1 = draggedNodeMeta.isLit ? toRGBA(draggedNodeMeta.visual.stroke, 0.32) : 'rgba(96,84,70,0.16)';
